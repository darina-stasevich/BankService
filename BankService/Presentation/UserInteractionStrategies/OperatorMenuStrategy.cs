...
public class OperatorMenuStrategy(
...
) : BaseMenuStrategy
{
    public override void ShowMenu()
    {
        ...
        Console.WriteLine("5. Revert user transaction");
    ...
    private Guid GetAccountId(int type)
    {
        while (true)
        {
            ...
            Console.WriteLine("1. Get {status} accounts");
            Console.WriteLine("0. Exit");
            Console.WriteLine("-1. Logout");
    ...
    private VerificationStatus GetVerificationStatus()
    {
        while (true)
        {
            var status = GetString("Enter verification status", false)!;
        ...
        switch (choice)
        {
            ...
            case 202:
            {
                ...
                else
                {
                    ...
                    {
                        Console.WriteLine($"- {account.Id}. UserID : {(account.UserAccountId != null?account.UserAccountId:"")} EnterpriseID : {(account.EnterpriseId != null?account.EnterpriseId:"")}");
            ...
            case 5: 
            {
                ...
                if(choiceRevertTransaction <= 0)
                    ...
                    HandleAuthenticatedInput(500 + choiceRevertTransaction);
                break;
            }
            case 501:
            {
                ...
                var transactionId = GetAccountId(502);
